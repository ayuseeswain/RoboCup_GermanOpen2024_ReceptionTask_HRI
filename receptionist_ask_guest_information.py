...
#The Receptionist task enables the HSR robot 'Lucy' to serve as a receptionist that can recognize and interact with guests using speech recognition technologies. It employs SpaCy NLP models to extract specific information like guest names and drink preferences from conversations. It incorporates a noise reduction library to filter out background noise in audio signals. 
...
class ReceptionistTask(ScenarioStateBase):
    ...
    def listen_and_transcribe(self):
        ...
        end_time = time.time()
        rospy.loginfo("Noise cleaning completed in {:.2f} seconds.".format(end_time - start_adjust))
        try:
